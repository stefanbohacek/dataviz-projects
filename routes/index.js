...
const getRequestPaths = (req) => {
  return {
    ...
    reqUrlFull: (
      ...
      req.get("host") +
      req.originalUrl
    ).replace("http://stefanbohacek.com/", "https://stefanbohacek.com/data/"),
  };
};
    ...
    if (fs.lstatSync(`projects/${project}`).isDirectory()) {
      ...
      if (fs.existsSync(configFilePath)) {
        ...
        projectList = projectList.concat(
          projectSubfolders.map((subfolder) => {
            if (
              fs.lstatSync(`projects/${project}/${subfolder}`).isDirectory()
            ) {
  ...
  if (req.query.format && req.query.format.toLowerCase() === "json") {
    ...
    res.render("../views/home.handlebars", {
      ...
      criticalCss: fs.readFileSync(
        `${__dirname}/../public/css/critical.min.css`,
        "utf8"
      ),
      timestamp: Date.now(),
    });
...
router.all("/projects/*", (req, res, next) => {
  ...
  if (
    projectPaths.project.indexOf("images") > -1 ||
    projectPaths.project.indexOf("data") > -1
  ) {
        ...
        try {
          let projectDataObject = {
            ...
            dependencies: fs.readFileSync(
              `${__dirname}/../projects/${projectPaths.project}/dependencies.html`,
              "utf8"
            ),
            projectURLencoded: encodeURI(
              `${projectPaths.appURL}projects/${projectPaths.project}`
            ),
            sc_project: process.env.SC_PROJECT,
            sc_security: process.env.SC_SECURITY,
            appURL: projectPaths.appURL,
          ...
          if (configJSON.data && configJSON.data.length) {
            configJSON.data.forEach((data) => {
              ...
              const fileName = dataPath[dataPath.length - 1]
                .split(".")[0]
                .replace(/-/g, "_");
              const fileExtension = dataPath[dataPath.length - 1]
                .split(".")
                .slice(-1);

              let fileData = fs.readFileSync(
                `${__dirname}/../public/data/${data}`,
                "utf8"
              );
          ...
          res.render(
            `${__dirname}/../projects/${projectPaths.project}/content.handlebars`,
            projectDataObject
          );
        } catch (err) {
          if (err.code === "ENOENT") {
