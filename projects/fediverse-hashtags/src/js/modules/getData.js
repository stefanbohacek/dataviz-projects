...
const getData = async () => {
  ...
  if (token && instance){
      ...
      if (statusCount){
        loadingStatusEl.innerHTML = `(<span id="progress-items">0</span>/${statusCount.toLocaleString()})`;
      }
      ...
      switch (platform) {
          ...
          case 'pleroma':
          case 'akkoma':
              statuses = await fetchData(instance, platform, `accounts/${userInfo.id}/statuses?limit=100`, token);
              break;
          case 'misskey':
          case 'calckey':
          case 'firefish':
          ...
          case 'magnetar':
              platform = 'misskey';
              statuses = await fetchData(instance, platform, `users/notes`, token, userInfo.id);
              break;
          default:
              break;
      }
